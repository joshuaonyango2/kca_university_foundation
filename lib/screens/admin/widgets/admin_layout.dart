...
class AdminLayout extends StatelessWidget {
  ...
  PreferredSizeWidget _buildMobileAppBar(BuildContext context) => AppBar(
    ...
    title: Text(title,
        style: const TextStyle(color: _KCA.white, fontWeight: FontWeight.bold)),
    actions: actions,
  );
            ...
            child: ClipOval(
              child: Image.asset('assets/image.asset.png',
                  ...
                  errorBuilder: (_, __, ___) => const Icon(
                      ...
                      color: _KCA.navy, size: 32)),
            ),
          ),
          const SizedBox(height: 12),
          const Text('KCA Foundation',
              style: TextStyle(
                  color: _KCA.white, fontWeight: FontWeight.bold, fontSize: 14)),
          const SizedBox(height: 4),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
            decoration: BoxDecoration(
                color: _KCA.gold, borderRadius: BorderRadius.circular(12)),
            child: const Text('Admin Portal',
                style: TextStyle(
                    color: _KCA.navy, fontSize: 11, fontWeight: FontWeight.w700)),
          ),
        ]),
      ),
        ...
        child: ListView(
          ...
          children: [
            ...
            _navItem(context,
                ...
                label: 'Dashboard', route: AppRoutes.adminDashboard),
            _navItem(context,
                icon: Icons.campaign_outlined, activeIcon: Icons.campaign,
                label: 'Campaigns', route: AppRoutes.adminCampaigns),
            _navItem(context,
                icon: Icons.people_outline, activeIcon: Icons.people,
                label: 'Donors', route: AppRoutes.adminDonors),
            _navItem(context,
                icon: Icons.receipt_long_outlined, activeIcon: Icons.receipt_long,
                label: 'Transactions', route: AppRoutes.adminTransactions),
            _navItem(context,
                icon: Icons.bar_chart_outlined, activeIcon: Icons.bar_chart,
                label: 'Reports', route: AppRoutes.adminReports),

            const SizedBox(height: 8),
            // ── Administration ────────────────────────────────────────────
            _sectionLabel('ADMINISTRATION'),
            _navItem(context,
                icon: Icons.group_outlined, activeIcon: Icons.group,
                label: 'Staff', route: AppRoutes.adminStaff),
            _navItem(context,
                icon: Icons.badge_outlined, activeIcon: Icons.badge,
                label: 'Roles & Permissions', route: AppRoutes.adminRoles),
            _navItem(context,
                icon: Icons.payment_outlined, activeIcon: Icons.payment,
                label: 'Payment Methods', route: AppRoutes.adminPaymentMethods),
            _navItem(context,
                icon: Icons.settings_outlined, activeIcon: Icons.settings,
                label: 'Settings', route: AppRoutes.adminSettings),
          ],
        ),
      ),
