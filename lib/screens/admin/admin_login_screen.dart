...
// ✅ FIX: removed unused 'package:firebase_auth/firebase_auth.dart' import (line 4 warning)
//    FirebaseAuth is used inside _routeAfterLogin via Provider<AuthProvider>,
//    not directly — so the direct import is unused and causes a lint warning.
...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Future<void> _routeAfterLogin(String uid, String email) async {
    if (PermissionService.isSuperAdminEmail(email)) {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        // ✅ Both methods now defined in NotificationService
        unawaited(NotificationService.subscribeAdminToTopic());
        unawaited(NotificationService.saveFCMToken(uid, isAdmin: isAdmin));
        ...
        await Provider.of<AuthProvider>(context, listen: false).logout();
        if (!mounted) return;
        final isSA = PermissionService.isSuperAdminEmail(email);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isSA
                ? 'Setting up your account… please log in again.'
                : 'Access denied. Your account has not been set up as staff.'),
            backgroundColor: Colors.red,
            behavior: SnackBarBehavior.floating,
          ),
        );
              ...
              return ElevatedButton(
                ...
                child: auth.isLoading
                    ...
                    child: CircularProgressIndicator(
                        strokeWidth: 2,
                        valueColor: AlwaysStoppedAnimation<Color>(_KCA.white)))
